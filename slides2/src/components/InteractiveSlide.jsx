    ...
    <Stack  id="modal_stack" p={0} spacing={0} sx={modalBoxStyle} direction="column">
      ...
      <Box id="modal_box" ref={boxRef} sx={{cursor:'grab'}}>
          <div ref={divRef} id="panzoom_div">
            {is_svg&&
                  <SVG src={`${config.base}/${src}`} id={image_id} onLoad={onLoad} />
            }
            {is_img&&
            <img src={`${config.base}/${src}`} id={image_id} onLoad={onLoad} draggable='false'/>
            }
          </div>
      </Box>
    </Stack>:
...
export default function InteractiveSlide({src,width=600,links}) {
  ...
  function closeModal(){
    if(false){const url = `${router.pathname}#pz-${src}`
